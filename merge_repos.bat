...
mkdir backend
git ls-tree -r --name-only HEAD | xargs -I files mv files backend/ 2>nul
git add .
...
mkdir admin
git ls-tree -r --name-only HEAD | xargs -I files mv files admin/ 2>nul
git add .
...
mkdir client
git ls-tree -r --name-only HEAD | xargs -I files mv files client/ 2>nul
git add .
